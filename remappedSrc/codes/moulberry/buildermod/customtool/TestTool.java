...
public class TestTool implements CustomTool  {
    ...
    public void leftClick() {
        if (targetPosition != null && MinecraftClient.getInstance().options.keyUse.isPressed()) {
    ...
    public void render(MatrixStack matrices, float tickDelta, Matrix4f projection) {
        ...
        Blueprint blueprint = BlueprintLibraryMenu.selected;
                ...
                for (int x = 0; x<blueprint.sizeX; x++) {
                    for (int y = 0; y<blueprint.sizeY; y++) {
                        for (int z = 0; z<blueprint.sizeZ; z++) {
                            BlockState state = blueprint.blockStates[x][y][z];
                            if (state != null) {
                                mutable.set(targetPosition.getX()+x-blueprint.pivotX,
                                        targetPosition.getY()+y-blueprint.pivotY, targetPosition.getZ()+z-blueprint.pivotZ);
