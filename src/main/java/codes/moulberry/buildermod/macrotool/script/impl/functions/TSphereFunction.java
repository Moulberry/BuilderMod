...
import codes.moulberry.buildermod.macrotool.script.impl.operations.mask.TBelowOp;
...
import codes.moulberry.buildermod.macrotool.script.impl.operations.mask.TNearOp;
    ...
    public static Grammar.rule getRule(ScriptParser parser) {
        if (rule == null) {
            Grammar.rule maskOperations = parser.choice(
                    ...
                    TBelowOp.getRule(parser),
                    TNearOp.getRule(parser)
            );
