...
public class SmoothTool extends GenericTool {
    ...
    protected void apply(Region region) {
        ...
        Long2ObjectMap<BlockPos> map = new Long2ObjectOpenHashMap<>(region.includedBlocks);
        LongSet settableBlocks = new LongOpenHashSet(region.includedBlocks.keySet());
    ...
    private boolean addNeighbor(Region region, World world, Long2ObjectMap<BlockPos> map, LongSet settableBlocks, BlockPos.Mutable mutable) {
        long id = mutable.asLong();
        if (region.includedBlocks.containsKey(id)) return true;
